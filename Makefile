...
pkgdatadir = $(datadir)/latinio
pkgincludedir = $(includedir)/latinio
pkglibdir = $(libdir)/latinio
pkglibexecdir = $(libexecdir)/latinio
...
CONFIG_HEADER = $(top_builddir)/src/config/latinio-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/latinio-config.h.in \
...
ACLOCAL = ${SHELL} /prj/coinFactory/works/bitcoin/latinio/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /prj/coinFactory/works/bitcoin/latinio/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /prj/coinFactory/works/bitcoin/latinio/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /prj/coinFactory/works/bitcoin/latinio/build-aux/missing automake-1.15
AVX2_CXXFLAGS = -mavx -mavx2
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = latinio-cli
BITCOIN_DAEMON_NAME = latiniod
BITCOIN_GUI_NAME = latinio-qt
BITCOIN_TX_NAME = latinio-tx
...
MAINT = 
MAKEINFO = ${SHELL} /prj/coinFactory/works/bitcoin/latinio/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = latinio
PACKAGE_BUGREPORT = latinio.io
PACKAGE_NAME = Latinio Core
PACKAGE_STRING = Latinio Core 1.0.0
PACKAGE_TARNAME = latinio
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /prj/coinFactory/works/bitcoin/latinio
abs_srcdir = /prj/coinFactory/works/bitcoin/latinio
abs_top_builddir = /prj/coinFactory/works/bitcoin/latinio
abs_top_srcdir = /prj/coinFactory/works/bitcoin/latinio
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /prj/coinFactory/works/bitcoin/latinio/build-aux/install-sh
...
DIST_CONTRIB = $(top_srcdir)/contrib/latinio-cli.bash-completion \
	       $(top_srcdir)/contrib/latinio-tx.bash-completion \
	       $(top_srcdir)/contrib/latiniod.bash-completion \
...
COVERAGE_INFO = baseline.info \
  test_latinio_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_latinio_coverage.info test_latinio.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Latinio-Core $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/latinio-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/latinio-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/latinio-config.h
$(top_srcdir)/src/config/latinio-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/latinio-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Latinio-Core: all-recursive
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Latinio-Core
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Latinio-Core: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_latinio.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_latinio -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_latinio_filtered.info: test_latinio.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_latinio_filtered.info
...
#test_latinio_coverage.info: baseline_filtered.info test_latinio_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_latinio_filtered.info -o $@

#total_coverage.info: test_latinio_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_latinio_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_latinio.coverage/.dirstamp:  test_latinio_coverage.info
...
#cov: test_latinio.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_latinio.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
