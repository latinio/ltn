...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::LTN);
    ...
    if (!settings.contains("nAnonymizeLTNAmount"))
        settings.setValue("nAnonymizeLTNAmount", 1000);

    nAnonymizeLTNAmount = settings.value("nAnonymizeLTNAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeLTNAmount"))
        SoftSetArg("-anonymizelatinioamount", settings.value("nAnonymizeLTNAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in latinio.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLTNAmount:
            return QVariant(nAnonymizeLTNAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeLTNAmount:
            nAnonymizeLTNAmount = value.toInt();
            settings.setValue("nAnonymizeLTNAmount", nAnonymizeLTNAmount);
            emit anonymizeLTNAmountChanged(nAnonymizeLTNAmount);
            break;
        case CoinControlFeatures:
