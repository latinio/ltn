...
#if defined(NDEBUG)
#error "LTN cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ulatinio) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < 1000) {
      nSubsidy = 55943.55;
    } else if (nHeight < 70794){
      nSubsidy = 100;
    } else if (nHeight < 386255){
      nSubsidy = 50;
    } else if (nHeight < 701616){
      nSubsidy = 25;
    } else if (nHeight < 1122097){
      nSubsidy = 12.5;
    } else if (nHeight < 1857938){
      nSubsidy = 6.25;
    } else if (nHeight < 3224499){
      nSubsidy = 3.125;
    } else if (nHeight < 6167860){
      nSubsidy = 1.5625;
    } else if (nHeight < 10372661){
      nSubsidy = 0.78125;
    } else if (nHeight < 18827062){
      nSubsidy = 0.390625;
    } else {
         nSubsidy = 0.390625;
    }
...
{
    RenameThread("latinio-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateLTNSupply(int nHeightStart)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // LTN: We use certain sporks during IBD, so check to see if they are
