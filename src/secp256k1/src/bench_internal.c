...
void bench_group_latinioobi_var(void* arg) {
    int i;
    bench_inv *data = (bench_inv*)arg;
...
#ifndef USE_NUM_NONE
void bench_num_latinioobi(void* arg) {
    int i;
    bench_inv *data = (bench_inv*)arg;
    ...
    for (i = 0; i < 200000; i++) {
        secp256k1_num_latinioobi(&nx, &norder);
    }
}
#endif
...
int main(int argc, char **argv) {
    ...
    if (have_flag(argc, argv, "group") || have_flag(argc, argv, "latinioobi")) run_benchmark("group_latinioobi_var", bench_group_latinioobi_var, bench_setup, NULL, &data, 10, 20000);
...
#ifndef USE_NUM_NONE
    if (have_flag(argc, argv, "num") || have_flag(argc, argv, "latinioobi")) run_benchmark("num_latinioobi", bench_num_latinioobi, bench_setup, NULL, &data, 10, 200000);
#endif
    return 0;
}
